...
float TemperatureSensorNl50::Read() const {
  ...
  const float millivolt = analogReadMilliVolts(pin_);
#elif defined(ARDUINO_ARCH_AVR)
  const uint8_t previous_analog_reference = analog_reference;
  analog_reference = kAnalogReference;
  const float millivolt = analogRead(pin_) * kReferenceMillivolt / 1023.0;
  analog_reference = previous_analog_reference;
#else
#error "unsupported arch"
#endif
  // Serial.println(String("millivolt: ") + millivolt + "mv");
  return millivolt / 10.0 - 50.0;
}
